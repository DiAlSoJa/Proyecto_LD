...
public class IniciarOperacionCommand : IRequest<Result<string>>
{
...
public class IniciarOperacionCommandHandler : IRequestHandler<IniciarOperacionCommand, Result<string>>
{
    private static readonly string[] TiposInicioOperacion = new[] { "IniciarOperacion", "ComenzarOperacion" };
    ...
    public IniciarOperacionCommandHandler(
    ...
    public async Task<Result<string>> Handle(IniciarOperacionCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            if (!tarea.Completada)
                return Result<string>.Failure("Primero debes abrir la cortina", []);

            tarea = await ObtenerOCrearTareaOperacionAsync(tarea.SecurityRegistrationId);
        }
    ...
    private async Task<SecurityTask> ObtenerOCrearTareaOperacionAsync(int securityRegistrationId)
    {
        var existente = (await _taskRepo.GetManyAsync() ?? [])
            .Where(t => t.SecurityRegistrationId == securityRegistrationId
                     && TiposInicioOperacion.Contains(t.TipoAccion))
